...
def get_file_paths_and_names(directory):
    file_paths = [os.path.join(directory, f) for f in os.listdir(directory) if os.path.isfile(os.path.join(directory, f)) and f[-4:]=='.pdf']
    file_names = [os.path.splitext(f)[0] for f in os.listdir(directory) if os.path.isfile(os.path.join(directory, f)) and f[-4:]=='.pdf']
...
if __name__ == "__main__":
    ...
    file_paths, file_names = get_file_paths_and_names("./data/chroma")

    combined_document_chunks = []
    for i in range(len(file_paths)):
        # Step 1: Parse PDF
        raw_pages, metadata = parse_pdf(file_paths[i])
        ...
        for item in document_chunks:
            combined_document_chunks.append(item)

    # Optional: Reduce embedding cost by only using the first 70 chunks
