...
def update_prices(init_df, allprice_df):
    ...
    init_df = init_df.merge(allprice_df, on=['basedate', 'Ticker'], how='left')
    init_df.drop(columns=['basedate', 'timestamp', 'high',
                 'low', 'volume', 'openinterest'], inplace=True)
...
def lowestLow(date1, ticker, status_dt, allp_df):
    ...
    results = pd.DataFrame({'NextDayDate': date1, 'Ticker': ticker, 'status_date': status_dt})
    results['LowestLow'] = results.apply(lambda row: getLow(row['NextDayDate'], row['Ticker'], row['status_date'], ticker_data), axis=1)
...
if __name__ == '__main__':
    ...
    input_files_list = glob.glob(f"{data_base_dir}/StockReading-2023/*-int.xlsx")

    def get_output_file_key(file):
        file_ = file.split('/')[-1].split('.')[0].lower().split('-')
        return f'{file_[1]}_{file_[2]}_file_op'

    output_files_dict = dict(map(lambda file: (get_output_file_key(file), file.replace('-int.xlsx', '-op.xlsx')), input_files_list))

    final_perf_summary = f"{data_base_dir}/StockReading-2023/StockReadingMetrics-perf-summary.xlsx"
    perf_dict_list = []
